...
public:
   ...
   /* string sense(Node* tmp) {
        stringstream rv;
        if (!tmp)return rv.str();
        rv << tmp->toString();
        if (!tmp->firstPtr)return rv.str();
        rv << " ";
        rv << sense(tmp->firstPtr);
        for (int i = 0; i < tmp->numEntries; i++)if (tmp->entries[i].rightPtr)rv << " " << sense(tmp->entries[i].rightPtr);
        return rv.str();
    }
    string toStringPreOrder() {
        return sense(root);
...
int main()
{
    ...
    int keys[] = { 78,21,14,11,97,85, 74, 63, 45, 42, 57, 20, 16, 19, 52, 30, 21 };// , 97, 85, 74, 63, 45, 42, 57, 20, 16, 19, 52, 30, 21};
    ...
    //cout << bTree.root->firstPtr->entries[0].key;
