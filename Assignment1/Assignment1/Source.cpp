...
FragmentLinkedList<T>::~FragmentLinkedList() {
	if (!empty()) {
		if (this->fragmentPointers[0]->next == NULL) {
			...
			this->tail = NULL;
			this->fragmentPointers[0] = NULL;
			for (int i = 0; i < ((this->count - 1) / this->fragmentMaxSize) + 2; i++) this->fragmentPointers[i] = NULL;
		}
	}
...
FragmentLinkedList<T>::Iterator::Iterator(FragmentLinkedList<T>* pList, int fragmentIndex, bool begin) {
	this->pList = pList;
	if (begin)	this->pNode = pList->fragmentPointers[fragmentIndex];
	else {
		if (fragmentIndex == -1)	this->pNode = NULL;
		else {
...
{
	if (this->pNode == NULL) return this->pList->fragmentPointers[0]->data;
...
{
	if (pNode == NULL) pNode = this->pList->fragmentPointers[0];
	else pNode = pNode->next;
	return *this;
}
...
{
	Iterator temp = *this;
	if (this== NULL)  *this = this->pList->begin(); 
	else ++(*this);  
	 return temp;
}
...
//	cout << fList.toString() << endl;
	...
	//for (FragmentLinkedList<int>::Iterator it = fList.begin(333); it != fList.end(334); it++)
	//	cout << *it << " ";
	//cout << endl;
	
	FragmentLinkedList<int> fList;
	for (int i = 1; i < 20; i++) fList.add(i);
	for (FragmentLinkedList<int>::Iterator v = fList.begin(); v != fList.end(); v++)
		if (*v % 3 == 0) {
			fList.removeItem(*v); v = fList.end();
		}
	cout << fList.toString();
	return 0;
