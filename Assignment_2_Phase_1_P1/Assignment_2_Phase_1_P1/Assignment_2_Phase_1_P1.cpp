...
int main()
{
	...
	vector<int> abcd;
	cout << tree->search(10, abcd);
	cout << tree->splay->search(10);
	tree->clear();
	/*tree->print();
	tree->remove(11);
	tree->print();
	...
	//tree->clear();
	int keysa[] = {10, 7, 14, 15, 12, 11, 6, 8, 9, 1, 50, 67, 102, 45, 74, 32, 27, 29, 56, 95, 100, 22, 76, 87, 89, 54, 69, 99, 88, 71, 73, 85, 90, 106, 121, 112, 83, 92};
	int n = sizeof(keysa) / sizeof(int);
	for (int i = 0; i < n; i++)
		tree->add(keysa[i], keysa[i]);
	tree->traverseNLROnAVL(printKey);
	tree->print();
	tree->remove(1);
	tree->remove(7);
